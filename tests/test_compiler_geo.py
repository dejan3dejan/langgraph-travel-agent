...
async def test_compiler_prompt_includes_preferred_areas(monkeypatch):
    # Named neighborhoods must reach the writer so days anchor there instead of a landmark checklist.
    captured = {}
    ...
    state = {
        "user_details": {
            ...
            "duration": "2 days",
            "needs_accommodation": False,
            "preferred_areas": ["Trastevere", "Prati"],
        },
        "activity_data": [{"name": "Colosseum", "address": "Rome", "lat": 41.89, "lon": 12.49}],
        "food_data": [],
        "hotel_data": [],
    ...
    assert "Trastevere" in captured["prompt"]
    assert "Prati" in captured["prompt"]
