...
judge_llm = ChatGoogleGenerativeAI(model="gemini-2.5-pro", google_api_key=os.getenv("GEMINI_API_KEY"), temperature=0)
...
class JudgeResponse(BaseModel):
    results: list[CriterionResult] = Field(
        description="List of results for each criterion provided, in the same order."
    )
...
async def run_simulation(scenario: dict) -> dict:
    ...
    while turn < max_turns:
        ...
        # Simulate User Input
        response, history, logs, _, _, _, _ = await orchestrator.chat(user_msg, history)
...
def grade_output(output: str, criteria: list[str]) -> dict:
    ...
    try:
        ...
        for i, status in enumerate(results):
            ...
            # Assign Weights
            if "destination" in criterion_text:
                weight = 40
            elif "duration" in criterion_text:
                weight = 20
            elif "budget" in criterion_text:
                weight = 20
            elif "start location" in criterion_text:
                weight = 5  # Less important
            else:
                weight = 15  # Hotels/Food
        ...
        return {"score": final_score, "explanation": ", ".join(breakdown)}
...
def print_summary_panel(metrics: MetricsAggregator):
    ...
    if geo["api_calls"] > 0:
        ...
        if total_attempts > 0:
            ...
            geo_table.add_row(
                "Neighborhood Fallback", str(geo["neighborhood"]), f"{geo['neighborhood']/total_attempts*100:.1f}%"
            )
